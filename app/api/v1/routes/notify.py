...
from app.api.schemas.response import ResponseEnvelope, ResponseFromTelegramLogic
...
async def notify_user(request: Request,
    ...
    try:
        result: ResponseFromTelegramLogic = await notify_user_by_bot(user_id=user_id, text=message)
        ...
        logger.critical(f'Ошибка уведомления пользователя: {e} \n{traceback.format_exc()}')
        return_content = ResponseEnvelope.error(
            request=request,
            message="Непредвиденная ошибка сервера при попытке уведомить пользователя.",
            http_status=500)
        return JSONResponse(status_code=500, content=return_content.model_dump())
